...
//Interfaces.. do I put this in schema?
interface Product {
  _id: string;
  prodNumber: string;
...
interface WithIndex extends Product {
  index: number,
  quantity: number,
}
...
export default function QuoteTable() {
  ...
  return (
    <>
    <main>    
      <div className="w-8/10 mx-auto rounded-md max-w-screen-lg"> {/* encapsulating div (80% width, rounded corners) */}
      <div className="flex justify-end py-3 print:hidden"> {/* User Input Component */}
        ...
        <Reorder.Group values={data} onReorder={setData}>
          <div className="grid grid-cols-[8%_12%_65%_10%_5%] rounded-md border border-gray-400"> {/* Grid container (5 columns, gap, border, rounded corners) */}
            ...
            <div className="px-2 py-1 text-center font-bold">Price</div>
          ...
          {data.map((item) => (
            <Reorder.Item key={item.index} value={item}>
              <div className="grid grid-cols-[8%_12%_65%_10%_5%] border-b border-gray-400"> {/* Row (grid, 5 columns, gap, bottom border) */}
                ...
                <div className="px-2 py-1 pl-5 text-left">${getPrice(item.prodListPrice, item.quantity)}</div>
